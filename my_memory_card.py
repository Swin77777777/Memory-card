...
lb_Question = QLabel('Здесь какой то вопрос')
...
def print_statistics():
    ...
    if window.total > 0:
        ...
        print('Рейтинг:', round(rating, 2), '%')
    print()

def update_window_title():
    if window.total > 0:
        rating = window.score / window.total * 100
        window.setWindowTitle(f'Memory Card - Рейтинг: {round(rating, 2)}%')
    else:
...
question_list = [
    ...
    Question('Как называется национальная хижина якутов', 'Ураса', 'Юрта', 'Иглу', 'Хата')
]
